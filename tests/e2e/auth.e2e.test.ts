...
describe.skipIf(!RUN_E2E)('auth E2E', () => {
  ...
  it('whoami --json returns non-empty id, name, email, workspace (shape only)', async () => {
  ...
  it('teams list succeeds with stored session (no explicit --token)', async () => {
  ...
  it('whoami with invalid --token exits non-zero with auth error', async () => {
