...
describe.skipIf(!RUN_E2E)('comments E2E', () => {
  ...
  it(
    'comment add --body --json returns comment with id, body, url',
    async () => {
      expect(issueId).not.toBe('');
      const body = `e2e root comment ${ts}`;

      const r = await runCLI(['issues', 'comment', 'add', issueId, '--body', body, '--json']);
      expect(r.code, `stdout: ${r.stdout}\nstderr: ${r.stderr}`).toBe(0);
      const data = r.json as {
        comment?: { id: string; body: string; url: string; createdAt: string; author: string };
      };
  ...
  it(
    'comment reply <commentId> --json returns reply',
    async () => {
  ...
  it(
    'comment update --body --json reflects changed body',
    async () => {
      expect(rootCommentId).not.toBe('');
  ...
  it(
    'comment list <issueId> --json returns array including root comment and reply',
    async () => {
      expect(issueId).not.toBe('');
      expect(rootCommentId).not.toBe('');
  ...
  it(
    'comment list --limit 1 --json returns at most 1 comment',
    async () => {
      expect(issueId).not.toBe('');
