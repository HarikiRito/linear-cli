...
export function registerCommentCommand(issues: Command): void {
  ...
  addAuthOptions(comment.command('list <issue>').description('List comments on an issue'))
    ...
    .action(
      async (
        issue: string,
        opts: { apiKey?: string; token?: string; limit: string; after?: string; json?: boolean }
      ) => {
        await listComments({
          apiKey: opts.apiKey,
          token: opts.token,
  ...
  addAuthOptions(
    ...
    .action(
      async (
        issue: string,
        opts: { body: string; apiKey?: string; token?: string; json?: boolean }
      ) => {
        await addComment({
          apiKey: opts.apiKey,
          token: opts.token,
