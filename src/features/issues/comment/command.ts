...
export function registerCommentCommand(issues: Command): void {
  ...
  addJsonOptions(listCmd).action(
    async (
      issue: string,
      opts: { apiKey?: string; token?: string; limit: string; after?: string; json?: boolean; pretty?: boolean }
    ) => {
      await listComments({
        apiKey: opts.apiKey,
        token: opts.token,
  ...
  addJsonOptions(addCmd).action(
    async (
      issue: string,
      opts: { body: string; apiKey?: string; token?: string; json?: boolean; pretty?: boolean }
    ) => {
      await addComment({
        apiKey: opts.apiKey,
        token: opts.token,
  ...
  addJsonOptions(replyCmd).action(
    async (
      commentId: string,
      opts: { body: string; apiKey?: string; token?: string; json?: boolean; pretty?: boolean }
    ) => {
  ...
  addJsonOptions(updateCmd).action(
    async (
      commentId: string,
      opts: { body: string; apiKey?: string; token?: string; json?: boolean; pretty?: boolean }
    ) => {
